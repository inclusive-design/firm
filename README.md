...
1. A generic UIO theme needs to be created which maps UIO primitves to different
   color tokens.
2. Dark mode and contrast themes need to be extracted into their own token files.
3. Unnecessary group prefixes need to be removed from variable names.
4. Leading and dimension values need to be properly formatted to
   match the DTCG spec.
5. Extraneous properties need to be removed.
6. The `$type` values need to be standardized to match the DTCG spec.
7. The updated tokens need to be written to their own files in the repo.
